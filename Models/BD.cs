...
public static class BD
{
    ...
    public static bool subirPublicacion(Publicacion publicacion)
    {
        ...
        {
            string query = @"
                INSERT INTO Publicaciones (IdUsuario, Descripcion, Foto, Precio, NombreProducto) 
                OUTPUT INSERTED.IdPublicacion
                VALUES (@pIdUsuario, @pDescripcion, @pFoto, @pPrecio, @pNombreProducto)
            ";
            
            try
            {
                connection.QuerySingle<int>(query, new
                {
    ...
    public static Publicacion RandomPublicacion()
    {
        using (SqlConnection connection = new SqlConnection(_connectionString))
        {
            string query = "SELECT TOP 1 * FROM Publicaciones ORDER BY NEWID()";
            return connection.QueryFirstOrDefault<Publicacion>(query);
        }
    }
    ...
    public static List<Publicacion> devolverPublicacionesVendedor(int idUsuario)
    {
        ...
        using (SqlConnection connection = new SqlConnection(_connectionString))
        {
            return connection.Query<Publicacion>(query, new { pidUsuario = idUsuario }).ToList();
        }
    }
    ...
    public static List<Publicacion> devolverPublicacionesPorLista(int idLista)
    {
        using (SqlConnection connection = new SqlConnection(_connectionString))
        {
            ...
            return connection.Query<Publicacion>(query, new { pIdLista = idLista }).ToList();
        }
    }
